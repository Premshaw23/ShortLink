      ...
      <div className="mb-6 flex flex-row items-center justify-center gap-4">
        <div className="bg-purple-50 border border-purple-200 rounded-xl px-5 py-3 text-purple-800 text-sm font-medium shadow-sm max-w-2xl">
          <span className="block mb-1">
            <span className="font-bold">Anonymous links</span> are links you
            created without signing in. They are only visible to you on this
            device and will expire in 24 hours.
          </span>
          <span className="block">
            <span className="font-bold">My links</span> are links you created
            while signed in. They are saved to your account and accessible from
            any device.
          </span>
        </div>
      </div>
              ...
              >
                <div className="mb-4 flex flex-col gap-2">
                  <div className="flex items-center gap-2 justify-between">
                    ...
                    {/* Ownership badge */}
                    <span
                      className={`ml-4 px-3 py-1 rounded-full text-xs font-bold whitespace-nowrap shadow-sm border ${
                        getLinkType(url) === "My Link"
                          ? "bg-green-100 text-green-700 border-green-200"
                          : getLinkType(url) === "Anonymous"
                          ? "bg-yellow-100 text-yellow-800 border-yellow-200"
                          : "bg-gray-100 text-gray-500 border-gray-200"
                      }`}
                      title={
                        getLinkType(url) === "My Link"
                          ? "This link belongs to your account"
                          : getLinkType(url) === "Anonymous"
                          ? "This is an anonymous link (only visible on this device)"
                          : "Other user's link"
                      }
                    >
                      {getLinkType(url)}
